...
class Timeline extends React.Component<any, any> {
  ...
  public render() {
    ...
    return (
      <div className="timeline">
        <svg id={SVG_NODE_ID} />
        {selectedEvent &&
          <div className="event-card" style={selectedEvent.right ? { top: selectedEvent.y, left: selectedEvent.x } : { top: selectedEvent.y, right: selectedEvent.x }}>
  ...
  private hoverEvent(event: Event, bonds: ClientRect, center: number, fromTitle: boolean) {
    const y = bonds.bottom + (fromTitle ? 17 : 0);
    this.setState({
      selectedEvent: (bonds.left + bonds.right) / 2 < center ? {
        event,
        right: true,
        x: bonds.left,
        y,
  ...
  private minimumExtentionFactor(events: [Event], pixelPerYear: number) {
    ...
    let isValid = false;
    do {
      gaps = events.map((event: Event, index: number) => index ?
        ((event.dateStart - events[index - 1].dateStart) / pixelPerYear) / factor
        : 0
      )
      isValid = !gaps.find(gap => gap > MIN_PIXEL_GAP);
      if (!isValid) { factor += factor / 10; }
    } while (!isValid);

    console.log('gaps', factor, gaps);
    return factor;
  }
  ...
  private draw() {
    ...
    const timeSpan = Math.abs((events[events.length - 1].dateEnd || events[events.length - 1].dateStart) - initDate);
    const pixelPerYear = timeSpan / innerWidth;

    this.minimumExtentionFactor(events, pixelPerYear);
    ...
    const nodes = select(svgNode)
      .selectAll("circle")
      .data(events)
      .attr("cx", (d: Event) => LAT_MARGINS + (d.dateStart - initDate) / pixelPerYear)
    ...
    const titles = select(svgNode)
      .selectAll(".svg-title")
      .data(events)
      .attr("x", (d: Event) => LAT_MARGINS + (d.dateStart - initDate) / pixelPerYear);
    ...
    titles.enter().append("text")
      ...
      .on('mouseover', (d: Event, index: number, items: [SVGElement]) => {
        ...
        this.hoverEvent(d, target.getBoundingClientRect(), fullDimensions.width / 2, true);
      });
    ...
    interactions.enter().append("rect")
      ...
      .attr("x", (d: Event) => LAT_MARGINS + (d.dateStart - initDate) / pixelPerYear - 50)
      .attr("width", 100)
      .attr("height", 100)
      .attr("fill", "red")
      .attr("fill-opacity", ".4")
      .on('mouseover', (d: Event, index: number, items: [SVGElement]) => {
        const target: SVGElement = items[index];
        this.hoverEvent(d, target.getBoundingClientRect(), fullDimensions.width / 2, false);
      });
